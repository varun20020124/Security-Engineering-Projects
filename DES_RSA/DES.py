...
class des():
    ...
    def substitute(self, d_e):#Substitute bytes using SBOX
        ...
        list_array=[]
        list_array = [self.compute_s_box(subblock, k + 1) for k, subblock in enumerate(subblocks)]
        result=[]
        result = [p for k in list_array for p in k]
    ...
    def compute_s_box(self,block,round):
        ...
        (1, 0): 2,
        (1, 1): 3}
        r = row_map.get((block[0],block[5]),0)
        t_block = block[1:5]
        bin = binvalue(S_BOX[round - 1][r][int(''.join(str(e) for e in t_block), 2)], 4)
        return bin
